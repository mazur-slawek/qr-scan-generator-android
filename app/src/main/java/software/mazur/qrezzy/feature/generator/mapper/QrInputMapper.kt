...
private fun QrInput.Email.toQrContent(): String {
    ...
    return buildString {
        append("MATMSG:")
        append("TO:$email;")
        append("SUB:${subject.trim()};")
        append("BODY:${body.trim()};;")
    }
}
...
private fun QrInput.Contact.toQrContent(): String {
    ...
    return buildString {
        ...
        appendLine("N:$contactLastName;$contactFirstName")
        appendLine("FN:${"$contactFirstName $contactLastName".trim()}")
        if (contactPhone.isNotBlank()) appendLine("TEL:$contactPhone")
