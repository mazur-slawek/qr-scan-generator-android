...
fun QrezzyQrPreview(qrBitmap: Bitmap?, modifier: Modifier = Modifier) {
    Card(
        ...
        elevation = CardDefaults.cardElevation(defaultElevation = QrezzyQrPreviewDefaults.Container.elevation)
    ) {
        _root_ide_package_.software.mazur.qrezzy.core.designsystem.components.QrezzyAnimatedStars(
            modifier = Modifier.fillMaxSize()) {
            AnimatedContent(
                targetState = qrBitmap,
                transitionSpec = {
                    fadeIn(animationSpec = tween(QrezzyQrPreviewDefaults.Animation.FADE_IN_DURATION_MILLIS))
                        .togetherWith(
                            fadeOut(
                                animationSpec = tween(QrezzyQrPreviewDefaults.Animation.FADE_OUT_DURATION_MILLIS)))
                        .using(SizeTransform(clip = false))
                },
                label = "qr_preview_animation"
            ) { bitmap ->
                bitmap?.let {
                    Image(
